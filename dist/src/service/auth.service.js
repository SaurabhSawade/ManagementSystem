"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const node_crypto_1 = __importDefault(require("node:crypto"));
const auth_model_1 = __importDefault(require("../model/auth.model"));
const httpStatus_1 = require("../constants/httpStatus");
const password_1 = __importDefault(require("../utils/password"));
const appError_1 = require("../utils/appError");
const otp_1 = __importDefault(require("../utils/otp"));
const env_1 = require("../config/env");
const jwt_1 = __importDefault(require("../utils/jwt"));
const logger_1 = require("../config/logger");
const login = async (username, password) => {
    const user = await auth_model_1.default.findUserByUsername(username);
    if (!user || !user.isActive || user.isBlocked) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.UNAUTHORIZED, "Invalid credentials", "AUTH_ERROR");
    }
    const valid = await password_1.default.comparePassword(password, user.passwordHash);
    if (!valid) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.UNAUTHORIZED, "Invalid credentials", "AUTH_ERROR");
    }
    const roles = await auth_model_1.default.getUserRoles(user.id);
    const jti = node_crypto_1.default.randomUUID();
    const payload = {
        userId: user.id,
        username: user.username,
        roles,
        jti,
        ...(user.email ? { email: user.email } : {}),
        ...(user.phone ? { phone: user.phone } : {}),
    };
    return {
        accessToken: jwt_1.default.signAccessToken(payload),
        refreshToken: jwt_1.default.signRefreshToken(payload),
        user: {
            id: user.id,
            username: user.username,
            email: user.email,
            phone: user.phone,
            roles,
        },
    };
};
const logout = async (accessToken) => {
    const payload = jwt_1.default.verifyAccessToken(accessToken);
    const tokenParts = accessToken.split(".");
    const payloadPart = tokenParts[1];
    const decoded = payloadPart
        ? JSON.parse(Buffer.from(payloadPart, "base64").toString())
        : {};
    const expiresAt = decoded.exp ? new Date(decoded.exp * 1000) : new Date(Date.now() + 15 * 60 * 1000);
    await auth_model_1.default.upsertTokenBlacklist({
        jti: payload.jti,
        tokenType: "ACCESS",
        userId: payload.userId,
        expiresAt,
    });
};
const refreshAuthTokens = async (refreshToken) => {
    const payload = jwt_1.default.verifyRefreshToken(refreshToken);
    const blacklisted = await auth_model_1.default.findTokenBlacklistByJti(payload.jti);
    if (blacklisted) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.UNAUTHORIZED, "Refresh token is blacklisted", "AUTH_ERROR");
    }
    const user = await auth_model_1.default.findUserById(payload.userId);
    if (!user || !user.isActive || user.isBlocked) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.UNAUTHORIZED, "Invalid refresh token", "AUTH_ERROR");
    }
    const roles = await auth_model_1.default.getUserRoles(user.id);
    const newJti = node_crypto_1.default.randomUUID();
    const newPayload = {
        userId: user.id,
        username: user.username,
        roles,
        jti: newJti,
        ...(user.email ? { email: user.email } : {}),
        ...(user.phone ? { phone: user.phone } : {}),
    };
    const refreshParts = refreshToken.split(".");
    const refreshPayload = refreshParts[1];
    const refreshDecoded = refreshPayload
        ? JSON.parse(Buffer.from(refreshPayload, "base64").toString())
        : {};
    const oldExpiry = refreshDecoded.exp
        ? new Date(refreshDecoded.exp * 1000)
        : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    await auth_model_1.default.upsertTokenBlacklist({
        jti: payload.jti,
        tokenType: "REFRESH",
        expiresAt: oldExpiry,
        userId: payload.userId,
    });
    return {
        accessToken: jwt_1.default.signAccessToken(newPayload),
        refreshToken: jwt_1.default.signRefreshToken(newPayload),
    };
};
const requestForgotPasswordOtp = async (params) => {
    const orFilters = [];
    if (params.email) {
        orFilters.push({ email: params.email });
    }
    if (params.phone) {
        orFilters.push({ phone: params.phone });
    }
    if (orFilters.length === 0) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "Email or phone is required", "VALIDATION_ERROR");
    }
    const user = await auth_model_1.default.findUserByContact(orFilters);
    if (!user) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.NOT_FOUND, "User not found", "NOT_FOUND");
    }
    const otp = otp_1.default.generateOtp();
    const otpHash = otp_1.default.hashOtp(otp);
    const expiresAt = new Date(Date.now() + env_1.env.OTP_EXPIRES_MINUTES * 60 * 1000);
    await auth_model_1.default.createOtpVerification({
        userId: user.id,
        email: user.email,
        phone: user.phone,
        channel: params.channel,
        codeHash: otpHash,
        purpose: "FORGOT_PASSWORD",
        expiresAt,
    });
    logger_1.logger.info(`OTP generated for ${user.username} using ${params.channel}: ${otp}`);
    return { masked: params.channel === "EMAIL" ? user.email : user.phone };
};
const verifyOtpAndResetPassword = async (params) => {
    const orFilters = [];
    if (params.email) {
        orFilters.push({ email: params.email });
    }
    if (params.phone) {
        orFilters.push({ phone: params.phone });
    }
    if (orFilters.length === 0) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "Email or phone is required", "VALIDATION_ERROR");
    }
    const otpRecord = await auth_model_1.default.findLatestOtp(orFilters);
    if (!otpRecord) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "OTP not found", "VALIDATION_ERROR");
    }
    if (otpRecord.expiresAt < new Date()) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "OTP expired", "VALIDATION_ERROR");
    }
    if (otpRecord.attempts >= env_1.env.OTP_MAX_ATTEMPTS) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.TOO_MANY_REQUESTS, "OTP attempts exceeded", "VALIDATION_ERROR");
    }
    const isValid = otpRecord.codeHash === otp_1.default.hashOtp(params.otp);
    await auth_model_1.default.incrementOtpAttempts(otpRecord.id, otpRecord.attempts + 1);
    if (!isValid) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "Invalid OTP", "VALIDATION_ERROR");
    }
    const user = await auth_model_1.default.findUserByContact([
        ...(params.email ? [{ email: params.email }] : []),
        ...(params.phone ? [{ phone: params.phone }] : []),
    ]);
    if (!user) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.NOT_FOUND, "User not found", "NOT_FOUND");
    }
    await auth_model_1.default.resetPasswordWithOtp({
        userId: user.id,
        otpRecordId: otpRecord.id,
        passwordHash: await password_1.default.hashPassword(params.newPassword),
    });
};
const resetMyPassword = async (params) => {
    const user = await auth_model_1.default.findUserById(params.userId);
    if (!user) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.NOT_FOUND, "User not found", "NOT_FOUND");
    }
    const valid = await password_1.default.comparePassword(params.currentPassword, user.passwordHash);
    if (!valid) {
        throw new appError_1.AppError(httpStatus_1.HTTP_STATUS.BAD_REQUEST, "Current password is incorrect", "VALIDATION_ERROR");
    }
    await auth_model_1.default.updateUserPassword(user.id, await password_1.default.hashPassword(params.newPassword));
};
const authService = {
    login,
    logout,
    refreshAuthTokens,
    requestForgotPasswordOtp,
    verifyOtpAndResetPassword,
    resetMyPassword,
};
exports.default = authService;
//# sourceMappingURL=auth.service.js.map